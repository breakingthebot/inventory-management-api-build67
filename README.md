...
| `GET` | `/api/v1/revisions` | Public / Viewer | List event-sourced entity revision logs (filter `?entity_class=`, `?entity_id=`) |
| `POST` | `/api/v1/revisions/{id}/rollback` | `ROLE_ADMIN` | Execute point-in-time state rollback to a past revision snapshot |
...
- **Audit Trail Engine**: `AuditTrailEngine` captures point-in-time state snapshots (`EntityRevision`) on mutation events and restores past entity states via `rollbackEntityToRevision()`.
